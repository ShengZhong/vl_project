...
const RecommendationsPage: React.FC = () => {
  ...
  const loadCategoryStats = async () => {
    try {
      const response = await getCategoryStats(params.platform);
  ...
  const loadRecommendations = async () => {
    ...
    try {
      ...
      if (response.code === 200) {
        ...
        console.error('加载建议列表失败:', response);
      }
    } catch (error) {
  ...
  // 脱敏显示账户ID
  const maskAccountId = (accountId: string | null | undefined): string => {
    if (!accountId) return '-';
  ...
  const renderCategoryStats = () => {
    return (
      <div 
        className="category-stats-scroll-container"
        style={{ 
          marginBottom: 24,
          overflowX: 'auto',
          overflowY: 'hidden',
        ...
        <div style={{ display: 'inline-flex', gap: 16, minWidth: '100%' }}>
          {categoryStats.map((stat) => (
            ...
            >
              <Card
                hoverable
                onClick={() => setParams({ ...params, category: stat.category.categoryCode, pageNum: 1 })}
                className={`category-card ${params.category === stat.category.categoryCode ? 'active' : ''}`}
                style={{ height: '100%' }}
              >
  ...
  return (
    <div className="recommendations-page">
      ...
      <Row gutter={16} style={{ marginBottom: 16 }}>
        <Col span={8}>
          <Search
            placeholder="搜索账户ID或建议..."
            allowClear
            onSearch={(value) => setParams({ ...params, keyword: value, pageNum: 1 })}
            style={{ width: '100%' }}
          />
        </Col>
        <Col span={4}>
        ...
        <Col span={4}>
          <Select
            ...
            onChange={(value) => setParams({ ...params, platform: value, pageNum: 1 })}
            style={{ width: '100%' }}
          >
          ...
          <div className="recommendation-detail">
            ...
            {currentRecommendation.account && (
              <div className="detail-section">
                <h3>账户信息</h3>
                <p>
                  <strong>账户ID:</strong> {maskAccountId(currentRecommendation.account.accountId)}
                </p>
                <p>
                  <strong>账户名称:</strong> {currentRecommendation.account.accountName || '-'}
                </p>
                <p>
                  <strong>机会分数:</strong> {renderOpportunityScore(currentRecommendation.account.opportunityScore)}
                </p>
              </div>
            )}
          </div>
