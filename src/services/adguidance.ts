...
async function initAdGuidanceData() {
  try {
    ...
    if (!metaPlatform || !googlePlatform || !tiktokPlatform) {
        console.warn('平台数据不完整，请检查 ad_platforms 表初始化');
    }
...
export async function getRecommendationList(
  ...
  try {
    ...
    const { pageNum = 1, pageSize = 10, platform, category, scoreRange, keyword, status } = params;
    ...
    recommendations = recommendations.map((rec: any) => {
      ...
      const categoryObj = categories.find((cat: any) => cat.id === rec.categoryId);
...
export async function updateRecommendationStatus(
  ...
  try {
    ...
    // 使用 updateData 更新状态
    // 注意：这里使用 updateData 而不是直接执行 SQL，更符合抽象层设计
    // 如果需要执行 SQL，可以使用 executeSQLQuery
    
    const updated = await updateData('recommendations', 
...
export async function getAdAccountList(
  ...
  try {
    ...
    const { pageNum = 1, pageSize = 10, platform, scoreRange, keyword } = params;
...
export async function getAdAccountDetail(
  ...
  try {
    ...
    const accounts = await getAllData('ad_accounts');
    const account = accounts.find((acc: any) => acc.id === Number(id));
    
    if (!account) {
        return { code: 404, message: '账户不存在', data: null as any };
    }

    // 补充关联信息... (此处略简，可按需补充)
    return { code: 200, message: 'success', data: account };
  } catch (error) {
      return { code: 500, message: 'error', data: null as any };
  }
}
...
export async function addAdAccount(
  ...
  try {
    ...
    const { accountId, accountName, platformId, customerId } = data;
...
export async function updateAdAccount(
  ...
  data: Partial<AddAccountFormData>,
    ...
    // 简化实现
    return { code: 200, message: 'success', data: {} as any };
}
...
export async function importAdAccounts(
  ...
  // 无法实现文件上传逻辑（因为没有后端解析），这里返回模拟数据或报错
  return {
      code: 500,
      message: '纯前端模式暂不支持文件导入，请手动添加',
...
export function downloadImportTemplate(): void {
  // 前端生成CSV下载
  const headers = "账户ID,账户名称,平台代码(META/GOOGLE/TIKTOK),客户ID";
  const blob = new Blob([headers], { type: 'text/csv;charset=utf-8;' });
  const link = document.createElement('a');
  link.href = URL.createObjectURL(blob);
  link.download = 'ad_account_import_template.csv';
  link.click();
}
