...
export async function getAdGuidanceList(
  ...
  try {
    const { pageNum = 1, pageSize = 20, consolidatedEntity, settlementEntity, adAccountId } = params;
...
export async function getRecommendationDetail(
  ...
  try {
    ...
    // 从数据库查询该账户的建议回传数据
    const recommendations = await findData<RecommendationDetail>(
...
export async function getMetricDetail(
  ...
  try {
    ...
    // 从数据库查询该账户的指标回传数据
    const metrics = await findData<MetricDetail>(
...
export async function addAccount(
  ...
  try {
    const { adAccountId, consolidatedEntity, settlementEntity, accountInfo, personnelInfo } = params;
