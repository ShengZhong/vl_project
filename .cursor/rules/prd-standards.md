...
2. [文档命名规范](#2-文档命名规范)
3. [完整文档模板](#3-完整文档模板)
4. [PRD 编写注意事项](#4-prd-编写注意事项)
5. [AI 生成 PRD 检查清单](#5-ai-生成-prd-检查清单)
...
| 规则 | 说明 | 示例 |
|------|------|------|
| **使用中文** | 便于团队沟通 | `PRD_账户开通管理.md` |
| **功能名称** | 简洁明确 | `PRD_素材批量上传.md` |
| **避免特殊字符** | 不使用空格、符号 | ✅ `PRD_用户管理.md` <br> ❌ `PRD-用户 管理.md` |
...
|------|------|
| **触发条件** | 用户提交的数据不符合校验规则 |
| **系统行为** | 在对应字段下方显示红色错误提示<br>保持弹框打开状态<br>高亮显示错误字段 |
| **用户操作** | 修改错误字段后重新提交 |
...
|------|------|
| **触发条件** | API 调用超时或返回错误 |
| **系统行为** | 显示全局错误提示："操作失败，请稍后重试"<br>记录错误日志<br>保留用户已填写的数据 |
| **用户操作** | 点击"重试"或关闭弹框 |
...
|------|------|
| **触发条件** | 用户无该功能的操作权限 |
| **系统行为** | 显示提示："您没有权限执行此操作，请联系管理员"<br>禁用相关按钮 |
| **用户操作** | 联系管理员分配权限 |
...
| **错误码** | 200: 成功<br>400: 参数错误<br>401: 未授权<br>500: 服务器错误 |
...
| **错误码** | 200: 成功<br>400: 参数错误<br>409: 账户名称已存在 |
...
| 禁止项 | 说明 |
|--------|------|
| ❌ **使用模糊描述** | 禁止："优化体验"、"更方便"、"更快"<br>应该：具体说明优化什么，如何量化 |
| ❌ **缺少功能点 ID** | 每个 PRD 必须有唯一的功能点 ID |
| ❌ **没有验收标准** | 必须提供清晰的功能、UI、性能验收标准 |
| ❌ **版本不一致** | PRD 更新时必须同步更新前端代码<br>在变更记录中注明 |

---

## 5. AI 生成 PRD 检查清单

在使用 AI 生成 PRD 时，必须确认以下内容：
